CLASS zcl_ks_data_uplaoder DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_ks_data_uplaoder IMPLEMENTATION.
  ...
  METHOD fill_master_data.
    DATA : lt_bp   TYPE TABLE OF zks_bpa,
           lt_prod TYPE TABLE OF zks_product.
    ...
    INSERT zks_bpa FROM TABLE @lt_bp.
    INSERT zks_product FROM TABLE @lt_prod.
  ENDMETHOD.
  ...
  METHOD fill_transaction_data.
    DATA : o_rand    TYPE REF TO cl_abap_random_int,
           ...
           lt_so     TYPE TABLE OF zks_so_hdr,
           lt_so_i   TYPE TABLE OF zks_so_itm.
    ...
    SELECT * FROM zks_bpa INTO TABLE @DATA(lt_bpa).
    SELECT * FROM zks_product INTO TABLE @DATA(lt_prod).
    ...
    INSERT zks_so_hdr FROM TABLE @lt_so.
    INSERT zks_so_itm FROM TABLE @lt_so_i.
  ENDMETHOD.

  METHOD flush.
    DELETE FROM : zks_bpa, zks_product, zks_so_hdr, zks_so_itm.
  ENDMETHOD.
